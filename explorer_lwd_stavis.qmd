...
AND/BUT...wary that drawing lots of fit lines doesn't mean they're any good...lots of looking at/for wood_change vs peak_flow led to below reexamination first of character of the wood_change

```{r dl1abs_vs_max_bnk_class}
d <- cfs_slp |> 
...
#starting to separate by comid before stepping back to reexamine dL1 itself
d |> 
  split(~strm) |> 
  `[`(x = _, j = c(1,3)) |>
  map(
...
## cleaning up per-year change object creation and per-stream viz of mapped and series
```{r wood_sy_sml_nwm}
...
is it interesting to examine years in which S changes opposite of M/L?
```{r wood_sy_sml_nwm_dy1}
wood_sy_sml_nwm_dy1 <- wood_sy_sml_nwm |> 
  #filter(is.na(bnk_wdth_mean)) #ABR004  2016
  mutate(
    #bnk_class = cut(mean(bnk_wdth_mean, na.rm = T), breaks = c(0,5,10,25)),
    across(
      starts_with("n"),
      list(
        dy1 = ~(. - lag(.))
...
```{r wood_sy_sml_nwm_dy1_smry}
wood_sy_sml_nwm_dy1_smry <- wood_sy_sml_nwm_dy1 |> 
  summarise(
    across(
      starts_with("n_"),
      list(
...
series are still difficult to make out with both M & L
could inline reduce set of sites beyond `site_meta` & `wood_sy_sml`
but some visually 'missing' change in series panels are actually just real 0s, e.g. BIB078

across sites and streams and years, seem to be fairly dramatic reversals?
most cases have M&L switching together (i.e., both up or both down) --> seems to exclude 'class shift' observation error as cause?
if not noticed first in Stavis, would seem like it could be project based additions that are then gone the next year...
  examples: 
    West: DEW057 13-14-15-16, DEW178 09-13, numerous sites 22(up)-23(down) not all adjacent (i.e from mass wasting or resto project)
    Stavis: 21-22 at 066, 006, 026, 025, 031; 23-24 at various
    Seabeck: 039, 001; others varied and 23-24
    Mill: 092, 025 in 2011-12
    Little: various in 23-24
    Germany: 090, 063 in 2013-14 and again 23-24
    East: 22-23 at 630, 356, 048 (with reversal again in 2024); 044; 19-20 at 003, 796, 444
```{r f_perstrm_dy1_points_map_series}
#wood_sy_sml_nwm_dy1 |> filter(site == "STA070") |> select( -contains("_S"))

map(
  names(pal_strm)
  ,
  \(s) {
    dy1 <- wood_sy_sml_nwm_dy1 |> filter(strm == s)
    
    dy1_smry <- wood_sy_sml_nwm_dy1_smry |> 
      filter(strm == s) |> 
      mutate(site = fct_reorder(site, n_L_m2_dy1_mean)) |> 
      #select(strm, site, contains("m2_dy1_mean")) |> 
      select(strm, site, contains("m2_dy1_")) |> 
      pivot_longer(-c(strm, site)) |> 
      mutate(value = 1000 * value) |> 
      separate(name, c('name', 'var'), sep = "1_") |> 
      pivot_wider(names_from = "var", values_from = "value") |> 
      filter(str_detect(name, "_S_", negate = T)) |> 
      mutate(
        wood_mean_change = case_when(
          mean > 0 ~ "increase",
          mean == 0 ~ "no_net_chg",
    ...
    #plot mean interannual change (not MAD!!!)
    site_mean_dy1 <- dy1_smry |> 
      ggplot() +
      geom_pointrange(
        aes(y = site, xmin = min, x = mean, xmax = max, group = site,
            #size = abs(mean),
            shape = mean > 0, #shape = name, 
            alpha = name,
        ),
        fatten = 1.5,
      ...
      scale_shape_manual(values = c(25,24)) + #"increase"=24, "decrease"=25
      labs(
        title = paste("Per-year change in wood piece density per site", s),
        subtitle = "sites ordered by mean change in largest size class; values are m^2 * 1000", y = NULL, x = "min-mean-max"
      ) + 
      theme(axis.text = element_text(size = 6))
    
    #plot basin map
    strm_mean_dy1 <- ggplot() +
      geom_sf(data = semi_join(sf_site_nhdp, dy1, by = "comid"), alpha = 0.5) +
      geom_sf(data = filter(sf_edges, strm == s), color = "lightblue") +
      geom_sf_text(data = filter(sf_site_meta, strm == s), aes(label = site), size = 3, nudge_y = 20) +
      geom_sf(
        data = inner_join(sf_site_meta, dy1_smry, by = c("strm","site"))
        ,
        mapping = aes(shape = wood_mean_change, fill = wood_mean_change, size = abs(mean))
    ...
    #plot site change series
    site_dy1_series <- dy1 |> 
      mutate(
        site = fct_reorder(site, n_L_m2_dy1, mean, .desc = T)
        ,year = factor(year)
        ) |> 
      select(site, year, n_M_m2_dy1, n_L_m2_dy1) |> 
      ...
      geom_col(aes(year, value, 
                   ...
                   fill = value < 0, 
                   alpha = name),
               position = position_dodge2(0.1), show.legend = F) +
      scale_fill_manual(values = c("#3A270A", "blue")) +
      scale_alpha_manual(values = c(0.8,0.4)) +
      facet_wrap(~site, ncol = 1, strip.position = "right") +
      #theme(strip.text = element_text(size = 5), axis.text = element_text(size = 5)) +
      theme(text = element_text(size = 8)) +
      labs(y = "m2_dy1", x = NULL)
    
    site_mean_dy1 + strm_mean_dy1 + site_dy1_series +
      plot_layout(widths = c(2,3.5,4.5)/1)
    
    ggsave(
      paste0("~/T/DFW-Team WDFW Watershed Synthesis - IMW analyses/fig/f_dy1_ML_point_map_series_by_strm_", str_split_i(s," ", 1),".png"),
      width = 20, height = 14, bg = "white", dpi = 100)
  }
)
...
since differencing introduces negative autocorrelation, not meaningful that many per-site series of differences are neg autocorr (just a property of randomness/white noise process)
and/but the frequency/consistency or magnitude of year-year up/down reversing could distinguish sites?
and/or larger magnitude reversals could be a lens on flows?
...
wood_sy_sml_nwm_dy1 |> 
  summarise(
    across(
      c(n_M_m2, n_L_m2
        #, n_M_m2_dy1, n_L_m2_dy1
        ),
      list(acf1 = ~acf(., lag.max = 3, na.action = na.pass, plot = F)$acf[2])
    )
    , 
    .by = c(strm, site)
  ) |> 
  pivot_longer(-c(strm, site)) |>  #filter(site == "MIL072")
  filter(abs(value) > 0.4) |> 
  arrange(value) |> #pull(site) -> tocheck
  ggplot() +
  geom_point(aes(name, site, shape = value > 0, fill = value > 0,, size = abs(value))) +
  scale_shape_manual(values = c(25,24))

#M (density) is indeed up-down-up-down...
wood_sy_sml_nwm_dy1 |> filter(site == "DEW021") |> select(site, year, contains("_M_"), contains("_L_"))
#L (density) same consistent reversing
wood_sy_sml_nwm_dy1 |> 
  filter(site == "MIL072") |>
  select(site, year, contains("_M_"), contains("_L_")) |> 
  ggplot() + geom_col(aes(year, n_L_m2))

#added DEW243, DEW067, AND002, GER004 to exclusions
#but others are mostly complete or 900series
wood_sy_sml_nwm |> 
  filter(site %in% tocheck) |>
  select(strm, site, year, contains("_m2"), -contains("_S_")
         ) |> 
  pivot_longer(-c(strm, site, year)) |> 
  ggplot() + geom_col(aes(year, value, fill = name), alpha = 0.6, position = position_dodge(0.4)) +
  facet_wrap(~site, nrow = 5, scales = "free_y")


```

what about MAD of density (by wood_class)?
```{r wood_sy_sml_nwm_dy1_mad}
#presumably could/should add to _smry object, but moving quick and not wanting to break pivots expecting only min/mean/max
wood_sy_sml_nwm_dy1_mad <- wood_sy_sml_nwm_dy1 |> 
  summarise(
    across(
      ends_with("_m2_dy1")
      ,
      list(
        mad = ~mean(abs(.), na.rm = T)
      )),
    .by = c(strm, site)
  ) |> 
  left_join(as_tibble(sf_edges) |> select(site, dasqkm_tot:upDist), by = "site")

#about what you'd expect? but nice demo of relative decoupling of S & L at least
wood_sy_sml_nwm_dy1_mad |> 
  GGally::ggpairs(columns = 3:5, aes(color = strm)) + scale_fill_manual(values = pal_strm, aesthetics = c("color","fill"))
#again, as expected, higher abs deviation of M density relative to L
wood_sy_sml_nwm_dy1_mad |> 
  ggplot() +
  geom_abline(slope = 1, intercept = 0) +
  geom_point(aes(n_L_m2_dy1_mad, n_M_m2_dy1_mad, color = strm)) + 
  scale_fill_manual(values = pal_strm, aesthetics = c("color","fill")) +
  facet_wrap(~strm)

#and now checking the obvious...
#LOOKS PROMISING?!?
#mix of signif/non-signif corr against DA & slope...
wood_sy_sml_nwm_dy1_mad |> 
  GGally::ggpairs(columns = 3:9, aes(color = strm), size = 0.7, alpha = 0.7) + scale_fill_manual(values = pal_strm, aesthetics = c("color","fill"))
```

MAD ~ slope: 

 - ST streams have clear positive reln (WTwin L_m2 r2 0.8!), stronger for L > M > S in Deep and West, looks leveraged for L in East by a steeper site with smaller magnitude diffs
 - HC and LC not nearly as clear/strong
  - Stavis L looks leveraged by STA002, but removing it only increases r2 slightly and worsens for M and S
  - likely other fitting options than just mad ~ slope

```{r wood_sy_sml_nwm_dy1_mad_slope}
wood_sy_sml_nwm_dy1_mad |> 
  #filter(str_detect(strm, "Stavis"), slope > 0.1) |> pull(site) -> tocheck #STA002, really that steep? 
  #filter(!(site %in% tocheck)) |> 
  select(strm, site, slope, ends_with("mad")) |> 
  pivot_longer(ends_with("mad")) |> 
  ggplot(aes(slope, value, color = strm)) +
...
MAD ~ DAsqkm
  ...
  - yay?! interestingly different picture
  - decently strong neg reln for M & S in HC, and arguably S across all streams
  - other than Mill and maybe Germany, little reln for L 
  
```{r wood_sy_sml_nwm_dy1_mad_dasqkm}
wood_sy_sml_nwm_dy1_mad |> 
  select(strm, site, dasqkm_tot, ends_with("mad")) |> 
  pivot_longer(ends_with("mad")) |> 
...
moving quickly for a first look

```{r wood_sy_sml_nwm_dy1_mad_lm}
wood_sy_sml_nwm_dy1_mad_lm <- wood_sy_sml_nwm_dy1_mad |> 
  nest(.by = strm) |> 
  mutate(
    lm_S = map(data, ~lm(n_S_m2_dy1_mad ~ dasqkm_tot + slope + elev_min + upDist, data = .x)),
    lm_M = map(data, ~lm(n_M_m2_dy1_mad ~ dasqkm_tot + slope + elev_min + upDist, data = .x)),
    lm_L = map(data, ~lm(n_L_m2_dy1_mad ~ dasqkm_tot + slope + elev_min + upDist, data = .x))
  )

wood_sy_sml_nwm_dy1_mad_lm |>
  mutate(
    lm_S_smry = map(lm_S, ~broom::glance(.x) |> select(r.squared, p.value) |> rename_with(~paste0("S_",.))),    
    lm_M_smry = map(lm_M, ~broom::glance(.x) |> select(r.squared, p.value) |> rename_with(~paste0("M_",.))),   
    lm_L_smry = map(lm_L, ~broom::glance(.x) |> select(r.squared, p.value) |> rename_with(~paste0("L_",.)))    
  ) |> unnest(ends_with("smry")) |> arrange(strm)

# wood_sy_sml_nwm_dy1_mad_lm |>
#   mutate(
#     lm_S_coef = map(lm_S, ~broom::tidy(.x) |> select(term, p.value) |> rename_with(~paste0("S_",.))),    
#     lm_M_coef = map(lm_M, ~broom::tidy(.x) |> select(term, p.value) |> rename_with(~paste0("M_",.))),   
#     lm_L_coef = map(lm_L, ~broom::tidy(.x) |> select(term, p.value) |> rename_with(~paste0("L_",.)))    
#   ) |> unnest(ends_with("coef")) |> arrange(strm) |> 
#   select(-data, -starts_with("lm")) |> pivot_wider(names_from = ends_with("term"), values_from = ends_with("value")) 



bind_rows(
  wood_sy_sml_nwm_dy1_mad_lm |>
    mutate(lm_S_coef = map(lm_S, ~broom::tidy(.x))) |> unnest(lm_S_coef) |> select(strm, term, p.value) |> mutate(wood_class = "S"),
  wood_sy_sml_nwm_dy1_mad_lm |>
    mutate(lm_M_coef = map(lm_M, ~broom::tidy(.x))) |> unnest(lm_M_coef) |> select(strm, term, p.value) |> mutate(wood_class = "M"),
  wood_sy_sml_nwm_dy1_mad_lm |>
    mutate(lm_L_coef = map(lm_L, ~broom::tidy(.x))) |> unnest(lm_L_coef) |> select(strm, term, p.value) |> mutate(wood_class = "L")
  ) |> 
#pivot_wider(names_from = term, values_from = `p.value`) |> 
  ggplot(aes(term, fct_rev(strm), 
             #size = 1-p.value, 
             size = cut(1-p.value, breaks = c(0, 0.9, 0.95, 0.99, 0.999, 1)),
             color = p.value < 0.05, 
             shape = p.value < 0.05
             )) +
  geom_point(show.legend = F) +
  #scale_size_binned(breaks = c(0, 0.9, 0.95, 0.99, 0.999, 1), range = c(0.2, 3)) +
  geom_text(aes(label = round(p.value, 4)), nudge_y = 0.2, size = 4, show.legend = F) +
  geom_text(
    data = wood_sy_sml_nwm_dy1_mad_lm |>
      mutate(
        lm_S_smry = map(lm_S, ~broom::glance(.x) |> select(r.squared, p.value) |> rename_with(~paste0("S_",.))),    
        lm_M_smry = map(lm_M, ~broom::glance(.x) |> select(r.squared, p.value) |> rename_with(~paste0("M_",.))),   
        lm_L_smry = map(lm_L, ~broom::glance(.x) |> select(r.squared, p.value) |> rename_with(~paste0("L_",.)))    
      ) |> unnest(ends_with("smry")) |> arrange(strm) |> select(strm, ends_with("squared")) |> 
      pivot_longer(-strm, names_to = "wood_class", values_to = "r2") |> 
      mutate(wood_class = str_sub(wood_class,1,1), r2 = round(r2, 2))
    ,
    aes(x = 3, y = fct_rev(strm), label = paste("r^2 =", r2)), inherit.aes = F, nudge_y = 0.4
  ) +
  scale_x_discrete(position = "top") +
  facet_wrap(~wood_class, nrow = 1) +
  labs(x = NULL, y = NULL) +
  theme(panel.background = element_rect(fill = "grey90"))

ggsave("~/T/DFW-Team WDFW Watershed Synthesis - IMW analyses/fig/f_dy1_mad_lm_physio_by_wood_class_strm.png", width = 11, height = 8, bg = "white", dpi = 150)

```



check simple sd - if pattern for diffs, should also be for standard dev? YES
[but in retrospect is arguably mostly pattern in mean, since sd strongly pos corr with mean]
```{r wood_sy_sml_nwm_sd}
wood_sy_sml_nwm_sd <- wood_sy_sml_nwm |> 
  left_join(site_meta |> select(cmplx_abrv, cmplx_strm, site), by = "site") |> 
  summarise(
    across(
      #ends_with("_m2"),
      ends_with("m2"), 
  ...
  mutate(
    ...
    across(ends_with("sd"), list(log = ~log(.)))
  ) |> 
  left_join(as_tibble(sf_edges) |> select(site, dasqkm_tot:upDist), by = "site") |> 
  arrange(strm)

# not quite as strong for some strm-woodclass combos, but definitely some apparently valid reln, esp for S
wood_sy_sml_nwm_sd |> 
  # select(strm, site, dasqkm_tot, ends_with("sd")) |>
  # pivot_longer(ends_with("sd")) |>
...
```{r wood_sy_sml_nwm_SD_lm}
wood_sy_sml_nwm_sd_lm <- wood_sy_sml_nwm_sd |> 
  nest(.by = strm) |> 
  mutate(
    lm_S = map(data, ~lm(n_S_m2_sd ~ dasqkm_tot + slope + elev_min + upDist, data = .x)),
    lm_M = map(data, ~lm(n_M_m2_sd ~ dasqkm_tot + slope + elev_min + upDist, data = .x)),
    lm_L = map(data, ~lm(n_L_m2_sd ~ dasqkm_tot + slope + elev_min + upDist, data = .x))
  )

#and yes, some plausible fits (stronger for S in several streams)
wood_sy_sml_nwm_sd_lm |>
  mutate(
    lm_S_smry = map(lm_S, ~broom::glance(.x) |> select(r.squared, p.value) |> rename_with(~paste0("S_",.))),    
    lm_M_smry = map(lm_M, ~broom::glance(.x) |> select(r.squared, p.value) |> rename_with(~paste0("M_",.))),   
    lm_L_smry = map(lm_L, ~broom::glance(.x) |> select(r.squared, p.value) |> rename_with(~paste0("L_",.)))    
...
ggsave("~/T/DFW-Team WDFW Watershed Synthesis - IMW analyses/fig/f_sd_lm_physio_by_wood_class_strm.png", width = 11, height = 8, bg = "white", dpi = 150)

## examining sample distrib for norm (plots contrast nonlogged with logged)
wood_sy_sml_nwm_sd |> 
  pivot_longer(starts_with("n_"), names_to = "wood_class", values_to = "val") |> #summary()
  filter(!is.infinite(val)) |> #drop BIB010 and BIB012 from 'L'
  ggplot(aes(fill = strm)) + 

  #  ggridges::geom_ridgeline(aes(val, strm, group = strm), alpha = 0.8, stat = "binline", scale = 0.5, bins = 40, show.legend = F) +
## nice/helpful
  #  geom_density(aes(val), alpha = 0.8, show.legend = F) +
## also nice/helpful
## log improves many/most but not in all streams for all wood_classes
  stat_qq(aes(sample = val, color = strm), show.legend = F) + stat_qq_line(aes(sample = val, color = strm), show.legend = F) +

  scale_fill_manual(values = pal_strm, aesthetics = c("color","fill")) +
  facet_wrap(~wood_class + strm, nrow = 6, labeller = label_wrap_gen(multi_line = F), scales = "free")

## switching to log response
wood_sy_sml_nwm_sd_log_lm <- wood_sy_sml_nwm_sd |>
  filter(!is.infinite(n_L_m2_sd_log)) |> #drop BIB010 and BIB012 from 'L'
  nest(.by = c(cmplx_abrv, cmplx_strm, strm)) |> 
  mutate(
    lm_S = map(data, ~lm(n_S_m2_sd_log ~ dasqkm_tot + slope + elev_min + upDist, data = .x)),
    lm_M = map(data, ~lm(n_M_m2_sd_log ~ dasqkm_tot + slope + elev_min + upDist, data = .x)),
    lm_L = map(data, ~lm(n_L_m2_sd_log ~ dasqkm_tot + slope + elev_min + upDist, data = .x))
  )

#still plausible fits, some arguably better without even considering distributional reasons
wood_sy_sml_nwm_sd_log_lm |>
  mutate(
    lm_S_smry = map(lm_S, ~broom::glance(.x) |> select(r.squared, p.value) |> rename_with(~paste0("S_",.))),    
    lm_M_smry = map(lm_M, ~broom::glance(.x) |> select(r.squared, p.value) |> rename_with(~paste0("M_",.))),   
    lm_L_smry = map(lm_L, ~broom::glance(.x) |> select(r.squared, p.value) |> rename_with(~paste0("L_",.)))    
...
wood_sy_sml_nwm_sd_log_lm |>
  mutate(
    #lm_fitted = map(lm_L, ~fitted(.x)), lm_resid = map(lm_L, ~residuals(.x))
    lm_fitted = map(lm_M, ~fitted(.x)), lm_resid = map(lm_M, ~residuals(.x))
    ) |> 
  unnest(c(lm_fitted, lm_resid)) |> 
  ggplot(aes(lm_fitted, lm_resid, color = strm)) + geom_point(show.legend = F) + 
  scale_fill_manual(values = pal_strm, aesthetics = c("color","fill")) +
  facet_wrap(~strm, nrow = 2, scales = "free")

#beautiful
plot(performance::check_model(wood_sy_sml_nwm_sd_log_lm$lm_L[[1]])) + plot_annotation(title = wood_sy_sml_nwm_sd_log_lm$strm[1])

#double map prevents double plotting before the reduced version
map(
  1:4, #nrow(wood_sy_sml_nwm_sd_log_lm),
  ~performance::check_model(wood_sy_sml_nwm_sd_log_lm$lm_L[[.x]]) 
) |>
  map(~plot(.x, n_columns = 6)) |> 
  reduce(`/`) + plot_annotation(tag_levels = list(rep(wood_sy_sml_nwm_sd_log_lm$strm[1:4], each = 6)))


```

trying various packages and formulas/model specifications, 
nothing actually usable but helpful to see and think about alternatives
```{r test_brms_hier_SD}
...
```{r wood_sy_sml_nwm_cmplt}
#need to expand_grid year to make missing explicit
#and drop SEA064 as apparently not linked to NHDplusHR 
#this creates many per-year repeats of the fixed site attributes da, slope, elev
(wood_sy_sml_nwm_cmplt <- wood_sy_sml_nwm |> 
   left_join(site_meta |> select(cmplx_abrv, cmplx_strm, site), by = "site") |> 
   ...
   #count(site)
   #summary() 
   #filter(is.na(dasqkm_tot))
   drop_na(dasqkm_tot) |> 
   arrange(cmplx_strm, site, year))
```

```{r test_brms_hier_negbin_ar}
...
```{r quick_site_mean_univariate_plots}
d |> 
  summarise(
    across(dasqkm_tot:upDist, first),
    across(ends_with("m2"), list(mean = ~mean(., na.rm = T))),
    .by = c(cmplx_strm, site)
  ) |> 
  GGally::ggpairs(
    aes(color = cmplx_strm, fill = cmplx_strm), columns = 3:9,
    diag = list(continuous = GGally::wrap("densityDiag", alpha = 0.5)),
    lower = list(continuous = GGally::wrap("points", alpha = 0.6, size = 0.6)),
...
d |> 
  ...
  pivot_longer(cols = ends_with("m2_mean"), names_to = "wood_class", values_to = "wood_m2") |> 
#  ggplot(aes(dasqkm_tot, wood_m2, color = cmplx_strm)) +
  ggplot(aes(slope, wood_m2, color = cmplx_strm)) +
...
#ggsave("~/T/DFW-Team WDFW Watershed Synthesis - IMW analyses/fig/f_wood_m2_da.png", width = 14, height = 11, dpi = 100, bg = "white")
ggsave("~/T/DFW-Team WDFW Watershed Synthesis - IMW analyses/fig/f_wood_m2_slope.png", width = 14, height = 11, dpi = 100, bg = "white")
...
sf_site_meta |> 
  ...
  inner_join(
    wood_sy_sml_nwm_sd |> select(site, ends_with("m2_sd"))
    , by = "site") |>
...
https://stats.stackexchange.com/q/151800 #nice description of 'random' effects

as usual a very nice treatment: https://otexts.com/fpp3/hierarchical.html
helpful for clarifying the question of whether or not site series aggregate (at least by summing) into a single 'per-stream' series
there is no 'total person' in a longitudinal study with repeat measures of subjects
not in the same sense as a 'total sales' from sectoral or geographic subdivisions


can read more on 'panel' (longitudinal) methods, with sites as subjects, nested in streams
https://www.econometrics-with-r.org/10-rwpd.html
https://www.econometrics-with-r.org/10.4-regression-with-time-fixed-effects.html


Next
  - assuming hierarchical (streams nested in complexes) in brms:
    - stick with log(sd) such that fitted response is the mean of the standard deviation
    - or more directly model the mean density (densities by stream) and examine the estimates of the sd?
    - or even more directly go back to modeling the per-year densities, maybe via ar(year)? treating as nested series as by Eric Ward (going back to something like https://atsa-es.github.io/atsa-labs/sec-marss-fitting-with-stan.html) 
  - any reason to re-examine median abs diff vs mean abs diff


prompts have useful text:

I'm a researcher analyzing a dataset with a response measure that is the standard deviation through time of per-year densities (i.e., the standard deviation of 15 years of count-per-area) at 25-30 sites across 10 different locations. Differences in this `sd` response indicate differences in the per-site volatility across years. The sample distributions across sites of this `sd` response are not perfectly Gaussian, but neither are they plainly bimodal or otherwise strongly non-normal. How would you suggest modeling this response? I'm especially interested in 1) what error distribution family to use and 2) whether to fit separately for each location or to pool with location as a fixed or random effect or to use a hierarchical structure. 

Those were both very helpful responses, thank you. In this analysis, the 25-30 sites are indeed nested within 10 distinct streams and their encompassing watersheds, but these 10 streams are further grouped within 3 'complexes' in different physiographic regions with known climatic, geologic, and land use variation. So the hierarchical structure, from top to bottom is actually 3 'complexes', with 10 streams split 3-3-4 across complexes, and with 25-30 sites per stream (and with 10-18 years of observations at each site, from which I'm calculating the per-site standard deviation). I've inspected QQ for log(sd), and it looks to bring most streams tolerably 'close to normal'. For now, assuming that I proceed with a log(sd) response, I'm interested in pursuing a hierarchical structure that includes site-level covariates for factors such as per-site slope (i.e., longitudinal gradient) and drainage area and elevation. I can see reasonably strong univariate linear relationships with several of my covariates, and separate models per stream of `log(sd) ~ dasqkm + slope + elev` have decent fit and performance statistics but suggest some nonlinearity (which is reasonable and expected). I know that there are "substantial, interpretable differences" in both response and covariates among complexes, among individual streams within complexes (the 'locations'), and among sites within streams. I'm interested in explaining patterns within these particular streams, but I'm also very interested in generalizing from them to the broader population of regional streams & watersheds. I'm quite open to a Bayesian approach (e.g., `rstanarm` or `brms` and/or something coded in `glmmTMB` or a different package. 

That was another very helpful answer, and I modified the examples to reach some preliminary model fits. Before I pursue those further, examining alternative formulations and interpreting the results, I'd like to step back and ask about a related but different approach. Since the response measure n_L_m2_sd_log is created by calculating the standard deviation over several years of density observations at each site, nested within each of the streams nested in complexes (with the densities themselves actually the count divided by the surveyed area), I would like to consider directly modeling those per-site annual series and evaluating the parameter estimates. The environmental covariates (dasqkm_tot, slope, elev_min) for individual sites are unchanged over time. Would a modified brm with a Poisson or negative binomial still make sense to account for temporal autocorrelation in the counts? If so, would that involve including an ar() term in the model formula? If not, what options would you suggest?
