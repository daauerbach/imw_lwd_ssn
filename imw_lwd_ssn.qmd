...
#rebuild flowline objects for LC
hr_gdb <- "~/T/DFW-Team WDFW Watershed Synthesis - data_common/nhdphr/17/NHDPLUS_H_1708_HU4_GDB.gdb"
st_layers(hr_gdb)
hr_h12 <- st_read(hr_gdb, layer = "WBDHU12") |> st_transform(st_crs(epsg))
hr_h12 <- hr_h12 |> #as_tibble() |> 
  select(HUC12, Name, HUType, AreaSqKm, ToHUC) |> 
  filter(str_detect(Name, "Mill Creek$|Germany|Abernathy")) #|> arrange(Name) |> print(n = Inf) 

hr_flw <- st_read(hr_gdb, layer = "NHDFlowline") |> 
  select(NHDPlusID, GNIS_Name, ReachCode, FCode, length_km = LengthKM) |> 
  st_zm() |> 
...
hr_flw_lc <- map(
  hr_h12$HUC12,
  ~hr_flw[filter(hr_h12, HUC12==.x),] |> mutate(HUC12 = .x)
  ) |> 
  bind_rows() |> 
  left_join(
    as_tibble(hr_h12) |> select(HUC12, Name),
    by = "HUC12") |> 
  left_join(
    st_read(hr_gdb, layer = "NHDPlusFlowlineVAA") |> #names()
    select(
      NHDPlusID, 
      #FromNode, ToNode,
      order = StreamOrde,
  ...
  left_join(
    st_read(hr_gdb, layer = "NHDPlusEROMMA") |> 
      select(NHDPlusID, QEMA, VEMA),
    by = "NHDPlusID"
  ) |> 
  mutate(
    cmplx_desc = "Lower Columbia",
    #h12 names are identical strings but not factors as 'strm' in existing IMW objs
    strm = factor(Name, levels = c("Mill Creek", "Abernathy Creek", "Germany Creek")), 
    across(starts_with("elev"), ~./100)
  )

ggplot() +  
  geom_sf(data = hr_h12, fill = NA) +
  geom_sf(data = hr_flw_lc, aes(linewidth = dasqkm_tot, color = QEMA)) +
  geom_sf(data = sf_nhdphr |> filter(cmplx_desc == "Lower Columbia"), color = "blue") +
  facet_wrap(~strm, ncol = 1)

saveRDS(hr_flw_lc, file = "~/T/DFW-Team WDFW Watershed Synthesis - IMW analyses/sf_hr_flw_lc.rds")
```
...
```{r ssnbler_starting_flowlines_and_sites}
sf_hr_flw_lc <- readRDS("~/T/DFW-Team WDFW Watershed Synthesis - IMW analyses/sf_hr_flw_lc.rds") |> 
  select(strm, HUC12, NHDPlusID, length_km, dasqkm_cat, dasqkm_tot, elev_min, elev_max, slope, QEMA, VEMA) |> 
  mutate(across(NHDPlusID, as.character))

sf_sites_lc <- site_meta_sf |> 
  filter(cmplx_abrv == "LC") |> 
  st_transform(st_crs(sf_hr_flw_lc)) |> 
  semi_join(
    as_tibble(site_meta_sf) |> 
      filter(year_n >= 10) |> 
      select(site, year_n)
    ,
    by = "site")
