...
The complex interplay among water, rocks, and trees is fundamental to habitat dynamics in rivers, generating the beautiful "messiness" that supports a rich and abundant riverscape biota (Scott et al. 2022). But oversimplification of these interactions through wood removal, channel straightening, and floodplain disconnection has degraded habitat, particularly in long-altered, temperate, forested watersheds. 

In recent decades, such degradation has prompted the active reintroduction of logs and various forms of engineered wood jams in the western United States, typically intended to benefit aquatic organisms via increased structural diversity (Roni et al. 2015, Ockelford et al. 2024). Much has been learned from such projects, and consensus has emerged around several general principles: avoid unsuitable locations, designs, and fixed targets for piece counts, while ensuring monitoring and follow-up, especially to address the secondary effects of initial interventions (e.g., Tables 3 and 4 in Ockelford et al. 2024). Yet practicable, quantitative management guidelines are still an area of active research, in part due to the logistical challenges of consistent measurement over many years at watershed extents (e.g., Hassan 2022, Martens & Devine 2022, Scott 2024, Ruiz-Villanueva et al. 2024).  

The 'natural wood regime' articulated by Wohl et al. (2019) offers a coherent theoretical framework to organize such guidance, describing recruitment, storage, and transport processes, each varying in magnitude, duration, frequency, timing, and rate of change. But the authors of this paradigm also note, "However, until more data are available to accurately parameterize mechanistic, multiscale models of wood regimes across regions (Scott and Wohl 2018), characterizing wood regimes over broad spatial scales will remain difficult" (Wohl et al. 2019).

This paper seeks to reduce uncertainties in how wood density varies in space and time throughout drainage networks by analyzing data collected between 2007 and 2024 at *NNN* consistently surveyed reaches within *NNN* small independent basins in western Washington, USA (Krueger et al. *in prep*). These data were collected as part of the Intensively Monitored Watersheds (IMW) program, a long-term, multi-location, collaborative evaluation of the population-level effects of habitat rehabilitation practices for Pacific salmon recovery (Bilby et al. 2005; Bennett et al. 2016). Physical and biological measurements made throughout 3-4 neighboring stream networks within each of three IMW 'complexes' enable watershed-scale comparisons of the factors affecting and limiting salmonids in forested basins of the Pacific Northwest (Roni 2005; Bilby et al. 2023; Krueger et al. *in prep*).

Within the broader, motivating question of "how can we most effectively define 'target wood regimes' (sensu Wohl et al. 2019) that are formulated to reverse impairments and meet ecosystem service goals?", we deploy a powerful spatiotemporal modeling approach to ask whether basic topographic attributes that are widely available in public hydrography datasets can consistently predict the "woodscape" of characteristic loads and fluxes. This "habitat distribution modeling" adapts the dendritic network species distribution modeling that can inform fisheries or other conservation management (Charsley et al. 2023, **MORE CITES?**).

Our approach accounts for ... and enables us to distinguish...

- spatial network attributes
- year or NWM, possible to distinguish hydroclim signal from other localized LULC processes?



## more wood in fluvial geomorph background?

Foundational studies have suggested [key] thresholds (e.g., density inflection at ~5m bankfull width)...but constrained by
  ...
  - Hassan et al. 2005
    ...
    - Fig2 (!!) Not have seen this previously is *highly congruent* with 'explorer_lwd' figures!! including 5m inflection noted in text but without the caveats of compiling across studies with varied protocol
      - can validate as: [Bilby & Ward 1989 density decay with width, central tendency] --> [Hassan et al 2005, also variance decay with width] --> [IMW shows both, multiple basins, several scales] --> and/but why 'hi vs lo' where var greatest in narrow site-reaches/network positions
    ...
    - attempt to explain some biogeographic variation in species composition (lifespan) and MAI (mean annual increment); low[er]land riparian longer lived, less disturbance prone, general expectation of lower inputs; then also note physical diffs of mature size trunks with lower/larger/longerlived also requiring considerably deeper flows to mobilize;
    - gaps: role of mass wasting; relative importance of depletion processes;
      - *Generally, ecosystem type affects the quality and quantity of wood available to a channel, while the spatial and temporal variability of wood recruitment, storage, and loss depends on variations in hydrology, forest type, and dominant erosion and transport processes. However, the effect of these variables as they change locally across a watershed (from valley bottom to mountaintop) or regionally across the landscape (from maritime to continental climates) is largely unknown and represents a third major knowledge gap.*

  - Fox & Bolton 2007
    - ref conditions from 150 sites, constraints of 'no upstream logging' and 'no other LU or hydro modification' fundamentally limits locations and scales...
    - explicitly targeting varied, stratified targets: *"A one-size-fits-all wood target approach may diminish habitat heterogeneity by reducing the natural range of wood conditions. Therefore, knowledge of the natural variation of instream wood loads among different stream types and regions should improve restoration activities as well as the scientific defensibility of regulatory thresholds."*
    - HMMM, opposite to Bilby, Hassan and IMW decay with larger BFW? *"The number and volume of instream wood per 100 m of channel length generally increase as drainage area increases (linear regression: P , 0.001) and as streams become less confined, particularly in watersheds greater than about 10 km2 in drainage area."*
    - "Bank-full width is supported as the most significant geomorphic indicator for predicting instream wood volumes and number of pieces" --> but again, this reln appears to be opposite that shown elsewhere of decreasing LWD with increasing BFW


  - Roni et al. 2015 Fact, fiction, future
    - Fig1 based on Abbe & Montgomery 2003 from Queets
  ...
  - Wohl et al 2019
    ...
    - "However, until more data are available to accurately parameterize mechanistic, multiscale models of wood regimes across regions (Scott and Wohl 2018), characterizing wood regimes over broad spatial scales will remain difficult"
  ...
  - Ockelford et al 2024 - working with wood in western US
    - Table 4 'likely ineffective' situations is nice summary: one and done, poor location or design, fixed targets, no consideration of secondary
...
Pending decisions about which basins and referencing Krueger et al. *in prep*

Large wood counts were made as part of the broader collection of stream habitat data in the western Washington IMW study. Annual surveys were conducted at sites within three complexes of 10 small, adjacent watersheds: the Strait of Juan de Fuca complex (ST) includes East Twin, West Twin (reference), and Deep creeks; the Hood Canal complex (HC) includes Little Anderson, Big Beef, Seabeck and Stavis (reference) creeks; the Lower Columbia complex (LC) includes Germany, Abernathy, and Mill (reference) creeks. Sampled site-reaches were identified using a random, spatially balanced design stratified by stream order (Stevens and Olsen 2004), with surveys conducted between June and October following standardized protocols based on established assessments (Lazorchak et al. 1998, Crawford et al. 2002, Roper et al. 2003). Data from 2007 onward are presented here. At each site in each year, measurements and counts are made at and between 21 equally spaced cross-sectional transects, positioned equidistant and perpendicular to flow along a length of stream that is the longer of either 40 bankfull widths or 300m. Measurements at each transect included bankfull width (to the nearest 0.0 m), water depth (≥5 points to the nearest 0.00 m), and tallies of substrate size classes. Wood within the bankfull channel was counted by length and diameter classes between transects. Substrate was classified at approximately 5 points per transect as either smooth bedrock or rough bedrock (> 4 m), boulder (250 – 4000 mm), cobble (64 – 250 mm), coarse gravel (16 – 64 mm), fine gravel (2 – 16 mm), sand (0.06 – 2 mm), fines (< 2 mm), hardpan, wood, or other. When found, dry channels were sampled and all possible measurements were made (e.g., depth is not measured).

*More detail on length and diameter classes and scoring and QAQC and standardization*
...
*Pending final VAST decisions, but generally 1) stratify to S/M/L 2) use SSNbler for network prep 3) use tinyVAST to fit 4) include NWM extension?*

  - is `nbinom2()` or even a poisson preferable?
    - https://github.com/vast-lib/tinyVAST/issues/60
  - is (1|site) sensible given the spatial field?
    - JH: could be, sort of roughly correspondent
  - multivariate response?
    - more in terms of sem writing S<->M<->L {pools/gravel/etc.}
  - continue to fit all streams separately or pool by complex via a dummy marine node?
  - what is the best way to differentiate space and time and to describe the differentiation?
    - how does the VAST 'spatial domain' graph of topology/spatial autoreg/autocorr(?) relate to the (spatially-influenced) covariates? 
    - are longitudinal hydrologic linkages actually being captured?
  - rather than any stepwise model selection within da/elev/slope, just examine intercept-only vs 3pred 'full' spatial vs with-year vs both year and 3pred?
    - model form alts
      1. intercept only
      2. da/elev/slope
      3. year only? (any point to '+year' vs s(year)? what about other AR process?)
      4. da/elev/slope + year



- restratification of counts to SML
  - supp table and figure
- 'steady state' spatial: (count | density) ~ bankfull ---> DA? + slope + elev
  - redo SSN with S-M-L response stratification; upArea corr with DA enough to drop?
- change relative to NWM
  - strat by SML --> S more responsive 90/50, M-L to max?
  - separate site series onto comid Q
  - corr of diff?
  - threshold (power?)
- 3 vs more non-ref basins

- HRCD/NLCD?

- Reduced to sites with complete or nearly complete series (BIB drops above Symington?)
- ACF, AR1 as well as or instead of CV? KPSS?
- corrr::correlate(method = "kendall") |> corrr::shave() |> corrr::rplot(print_cor = T) + scale_x_discrete(guide = guide_axis(n.dodge = 2))


# Results

- summarize landscape-scale numbers with `predict(newdata = other_NHDplusHR_basins)`?

# Discussion

- wood and its interactions with flow and sediment are fundamental to riverscape habitat dynamics

- reducing uncertainty in the spatiotemporal variation  is necessary to reach a more systematized legal and policy framework of 'target wood regimes' formulated to reverse impairments and meet other ecosystem service goals (Wohl et al. 2019), in the context of existing instream or environmental flow rules and in conjunction with riparian and upland land management practices.
...
scraps
 - A general pathologic condition has been identified (lack of wood), and a therapeutic approach has been proposed and adopted (add wood), but much remains to be learned about exactly what doses, delivered when, where and how will provide the most benefit across the range of individual watershed 'patients'.
 - Current best practice in stream rehabilitation seeks to identify 'limiting factors' and tailor 'process-based' interventions to ameliorate them.
...
Thorson, J. T., Anderson, S. C., Goddard, P., & Rooper, C. N. (2025). tinyVAST: R package with an expressive interface to specify lagged and simultaneous effects in multivariate spatio-temporal models. Global Ecology and Biogeography. 34(4): e70035. https://doi.org/10.1111/geb.70035
...
op <- vast_fit |> 
  mutate(
    across(contains("_f"), list(pred = ~map(., ~.x$rep$mu_i)))
  ) |> 
  select(d_S, ends_with("pred")) |> 
  unnest(everything()) |>
  select(cmplx_strm:site, dasqkm_tot, year, obs = n, ends_with("_pred")) |> 
  pivot_longer(c(ends_with("_pred")), names_to = "alt", values_to = "pred")

op |> 
  ggplot() +
  geom_point(aes(obs, pred, color = alt, group = site), alpha = 0.5, size = 0.7) +
  geom_abline(intercept = 0) +
  wacolors::scale_color_wa_d() +
  #facet_wrap(~cmplx_strm, scales = "free", nrow = 2)
##needs big monitor and should prob split by strm to keep y-scale fixed across alts
  facet_wrap(~cmplx_strm + alt, labeller = label_wrap_gen(multi_line = F), scales = "free", ncol = 3)

op |> 
  ggplot() +
  ggridges::geom_density_ridges(aes(obs, site, fill = cmplx_strm)) +
  ggridges::geom_density_ridges(aes(pred, site, fill = cmplx_strm, alpha = alt)) +
  scale_fill_manual(values = pal_cmplx_strm, aesthetics = c("color","fill")) + 
  facet_wrap(~cmplx_strm, scales = "free", nrow = 1)
...
wrap_vast <- function(
    ...
    dist_col = "dist_col",
    s_term = ""
    #will want time_term and spacetime_term
    #t_term
    #st_term
    ){
  try(tinyVAST(
...
vast_fit <- tibble(
  ...
  mutate(
    ...
    d_M = map(
      cmplx_strm,
      ~as.data.frame(vast_strm_d) |> 
        filter(cmplx_strm == .x, wood_class == "M") |> 
        droplevels()
    ),
    d_L = map(
      cmplx_strm,
      ~as.data.frame(vast_strm_d) |> 
        filter(cmplx_strm == .x, wood_class == "L") |> 
        droplevels()
    )
    ,
    d_S_f_smth_yr = map2(
      d_S, g, ~wrap_vast(d = .x, g = .y, f = n ~ 1 + s(year) + offset(log(lgth_wdth)))
    ),
    d_M_f_smth_yr = map2(
      d_M, g, ~wrap_vast(d = .x, g = .y, f = n ~ 1 + s(year) + offset(log(lgth_wdth)))
    ),
    d_L_f_smth_yr = map2(
      d_L, g, ~wrap_vast(d = .x, g = .y, f = n ~ 1 + s(year) + offset(log(lgth_wdth)))
    )
  )



#large wood_class fits not okay for BBC, LA, Stavis
vast_fit |> 
  mutate(
    across(
      contains("_f_")
      , 
      list(
        pdH = ~map_dbl(., ~if(class(.x)=="tinyVAST"){.x$sdrep$pdHess}else{NA_real_})
      )
    )
  )


#still need means to compare among streams/dataset sizes
vast_fit |> 
  mutate(
    across(
      c(contains("_S_f_"), contains("_M_f_"))
      #contains("_f_")
      , 
      list(
        pdH = ~map_dbl(., ~if(class(.x)=="tinyVAST"){.x$sdrep$pdHess}else{NA_real_}),
        devxpl = ~map_dbl(., ~if(class(.x)=="tinyVAST"){.x$deviance_explained}else{NA_real_}),
        aic = ~map_dbl(., ~if(class(.x)=="tinyVAST"){AIC(.x)}else{NA_real_}),
        caic = ~map_dbl(., ~if(class(.x)=="tinyVAST"){cAIC(.x)}else{NA_real_})
      )
    )
  ) |> 
  select(cmplx_strm, contains("yr_"))

map(vast_fit$d_S_f_smth_yr, ~.x$sdrep)
map(vast_fit$d_M_f_smth_yr, ~.x$sdrep)


op <- vast_fit |> 
  mutate(
    across(contains("_S_f_"), list(pred = ~map(., ~.x$rep$mu_i)))
  ) |> 
  select(d_S, ends_with("pred")) |> 
  unnest(everything()) |>
  select(cmplx_strm:site, dasqkm_tot, year, obs = n, ends_with("_pred")) |> 
...
op |> 
  ...
  wacolors::scale_color_wa_d() +
  #facet_wrap(~cmplx_strm, scales = "free", nrow = 2)
  facet_wrap(~cmplx_strm+site, labeller=label_wrap_gen(multi_line = F), scales = "free") + theme(text = element_text(size = 4))

op |> 
  ggplot() +
  ggridges::geom_density_ridges(aes(obs, site, fill = cmplx_strm), alpha = 0.8) +
  ggridges::geom_density_ridges(aes(pred, site, fill = cmplx_strm), alpha = 0.3) +
  scale_fill_manual(values = pal_cmplx_strm, aesthetics = c("color","fill")) + 
  facet_wrap(~cmplx_strm, scales = "free", nrow = 1) +
...
#    me = mean(rr),
  ...
  left_join(
    vast_strm_d |> distinct(site, dasqkm_tot, elev_min, slope), by = "site"
  ) |> 
  # ggplot() +
  # #geom_point(aes(mae, rmse)) +
  # #geom_point(aes(dasqkm_tot, rmse)) +
  # geom_point(aes(dasqkm_tot, slope, size = rmse), alpha = 0.5) + scale_size_binned_area() +
  # facet_wrap(~cmplx_strm, scales = "free", nrow = 2)

  GGally::ggpairs(columns = 3:7, mapping = aes(color = cmplx_strm, group = site, alpha = 0.5)) +  scale_fill_manual(values = pal_cmplx_strm, aesthetics = c("color","fill"))

#interesting how clearly the ABE resto sites show up
sf_site_meta |> 
  select(1:6) |> 
  inner_join(
    op |> 
      mutate(rr = obs - pred) |> 
      summarise(mae = mean(abs(rr)), .by = c(site))
    , by = "site"
  ) |> 
  split(~cmplx_abrv) |> 
  map(
    ~.x |> ggplot() + 
      geom_sf(data = filter(sf_edges, str_detect(cmplx_strm, str_sub(.x$cmplx_strm[1],1,2))), linewidth = 0.5, color = "lightblue") +
...
matrix(c(
  ...
  "M","parr/spawner",
  "L","parr/spawner",
  "pool_depth","parr/spawner"
), ncol = 2, byrow = T) |> 
  igraph::graph_from_edgelist() |> 
  igraph::add_layout_(igraph::as_tree()) |> #igraph::V()
  igraph::plot.igraph(
    vertex.color = c("grey","tan","tan","tan","grey",alpha("darkgreen",0.6),"blue","salmon")
    )

```

more testing:
 - passing multiple wood_class levels of 'variable_column' [works]
 - aggregating to S and ML via rowsplit and bind, better to do earlier if going that route

```{r try_multi_aggreg_class}
library(tinyVAST)

d_test = droplevels(as.data.frame(filter(vast_strm_d, str_detect(cmplx_strm, "West"))))
g_test = vast_strm_g$`ST_West Twin River*`

d_test = droplevels(as.data.frame(filter(vast_strm_d, str_detect(cmplx_strm, "Mill"))))
g_test = vast_strm_g$`LC_Mill Creek*`

d_test = droplevels(as.data.frame(filter(vast_strm_d, str_detect(cmplx_strm, "Stavis"))))
g_test = vast_strm_g$`HC_Stavis Creek*`

d_test <- bind_rows(
  filter(d_test, wood_class == "S")
  ,
  filter(d_test, wood_class != "S") |> 
    mutate(wood_class = "ML", n = sum(n), .by = c(site, year)) |> 
    distinct(site, year, .keep_all = T)
) |> 
  mutate(wood_class = factor(wood_class))

summary(d_test)
summarize(d_test, across(n, list(min = ~min(.), mu = ~mean(.), sd = ~sd(.), v = ~var(.))), .by = wood_class) ggplot(d_test) + geom_histogram(aes(n, fill = wood_class)) + facet_wrap(~wood_class, scales = "free",ncol = 1)
ggplot(d_test) + geom_density(aes(n, fill = wood_class)) + facet_wrap(~wood_class, scales = "free", ncol = 1)
ggplot(d_test) + geom_qq(aes(sample = n)) + geom_qq_line(aes(sample = n)) + facet_wrap(~wood_class, scales = "free")

ggplot(d_test) + geom_density(aes(log(n), fill = wood_class)) + facet_wrap(~wood_class, scales = "free", ncol = 1)

f1 <- tinyVAST(
  data = d_test #|> mutate(n = log1p(n)) #|> summary()
  , 
  spatial_domain = g_test,
  space_column = c("X","Y"),
  variable_column = "wood_class",
  time_column =  "year",
  distribution_column = "dist_col",
  space_term = "", #t_term #st_term

  #formula = n ~ 0 + log10(dasqkm_tot) + elev_min + slope + offset(log(lgth_wdth))
  #formula = n ~ 0 + log10(dasqkm_tot) + elev_min + slope + s(year) + offset(log(lgth_wdth))
  #formula = n ~ 0 + log10(dasqkm_tot) + elev_min + slope + s(year, by = wood_class) + offset(log(lgth_wdth))
  #formula = n ~ 0 + log10(dasqkm_tot) + elev_min + slope + wood_class + offset(log(lgth_wdth))
  #formula = n ~ 0 + log10(dasqkm_tot) + elev_min + slope + wood_class + s(year) + offset(log(lgth_wdth))
  #formula = n ~ 0 + log10(dasqkm_tot) + elev_min + slope + wood_class + s(year, by = wood_class) + offset(log(lgth_wdth))
##very marginally better cAIC and prop dev expl for fixed log10(da) vs s(da)
  #formula = n ~ 0 + s(dasqkm_tot) + wood_class + s(year, by = wood_class) + offset(log(lgth_wdth))
  #formula = n ~ 0 + log10(dasqkm_tot) + wood_class + s(year, by = wood_class) + offset(log(lgth_wdth))
  formula = n ~ 0 + log10(dasqkm_tot) + s(year, by = wood_class) + offset(log(lgth_wdth))
  ,
  #family = nbinom1() #better than nbinom2() at least for Stavis, S-ML
  family = poisson() #everything but 'deviance explained' says this is worse
  #family = lognormal()

  # family = delta_lognormal(type = "poisson-link"),
  # delta_options = list(
  #   formula = ~ 0 + log10(dasqkm_tot) + s(year, by = wood_class) + offset(log(lgth_wdth)),
  #   space_term = ""
  #   #,spacetime_term = ""
  #   )
...
f1; cAIC(f1); AIC(f1)
logLik(f1)
#negbin -4405.573 (df=7)
#pois -8399.578 (df=6)

glm(n ~ 0 + log10(dasqkm_tot) + offset(log(lgth_wdth)), data = d_test, family = poisson) |> summary()
MASS::glm.nb(n ~ 0 + log10(dasqkm_tot) + offset(log(lgth_wdth)), data = d_test) |> summary()

#zero inflation not a concern when aggregating M+L
#but still potential overdispersion
#try log1p(n) and family = lognormal?
d_test |> count(wood_class, n>0)
bind_rows(
  filter(vast_strm_d, wood_class == "S")
  ,
  filter(vast_strm_d, wood_class != "S") |> 
    mutate(wood_class = "ML", n = sum(n), .by = c(site, year)) |> 
    distinct(site, year, .keep_all = T)
) |> 
  count(wood_class, n>0)
...
s <- st_read(
  file.path(
    system.file("stream_network",package="tinyVAST"),
...
alpha = 2
kappa = 0.05
omega_s = tinyVAST::simulate_sfnetwork(n=1, sfnetwork_mesh=g, theta=kappa)[,1]
#reduce to fewer sampled sites than orig example
extrap = as_tibble(s, "edges") |>
  st_line_sample(n = 1) |> #one per linestring
  st_union() |> st_cast("POINT")
extrap = extrap[sample(1:length(extrap), size = 25, replace = F)]
A_is = tinyVAST::sfnetwork_evaluator(
  stream = g$stream,
  loc = st_coordinates(extrap)
  )
omega_i = (A_is %*% omega_s)[,1]
summary(omega_i); summary(alpha + omega_i)
Count_pois = rpois(n=length(omega_i), lambda=exp(alpha + omega_i))
Count_nb = rnbinom(n=length(omega_i), mu=exp(alpha + omega_i), size = .1) #tried a few 'size' values
summary(Count_pois); hist(Count_pois)
summary(Count_nb); hist(Count_nb)

ggplot() + 
  geom_sf(data = as_tibble(g$stream, "nodes") |> mutate(omega_s = omega_s), aes(color = omega_s)) +
  geom_sf(data = as_tibble(g$stream, "edges") |> mutate(omega_s = omega_s[from]), aes(color = omega_s)) +
  geom_sf(data = st_as_sf(extrap) |> mutate(n = Count_pois), aes(size = n, fill = n), shape = 24, color = "grey") +
  scale_size_area(max_size = 4)

Data_pois = data.frame(
  Count = Count_pois, st_coordinates(extrap),
  var = "species", time = "2020", dist = "obs")   
Data_nb = data.frame(
  Count = Count_nb, st_coordinates(extrap),
  var = "species", time = "2020", dist = "obs")   

tibble(
  sim_count = c("pois","nb"),
  d = list(Data_pois, Data_nb)
  ) |> 
  mutate(
    fit_pois = map(d, ~tinyVAST::tinyVAST(
      data = .x, family = poisson(),
      formula = Count ~ 1,
      spatial_domain = g, space_column = c("X","Y"),
      variable_column = "var", time_column = "time",
      distribution_column = "dist",
      space_term = ""))
    ,
    fit_nb1 = map(d, ~tinyVAST::tinyVAST(
      data = .x, family = nbinom1(),
      formula = Count ~ 1,
      spatial_domain = g, space_column = c("X","Y"),
      variable_column = "var", time_column = "time",
      distribution_column = "dist",
      space_term = ""))
    ,
    across(c(fit_pois, fit_nb1), list(cAIC = ~map_dbl(., ~cAIC(.x))))
    ,
    across(c(fit_pois, fit_nb1), list(devExp = ~map_dbl(., ~.x$deviance_explained)))
  )
...
d <- vast_strm_d |> 
  select(cmplx_strm:site, year, lgth_wdth, wood_class, n) |> 
  group_by(cmplx_strm, cmplx_abrv, cmplx_desc, strm, site, wood_class) |> 
  complete(year = 2007:2024) |> 
  ungroup()
...
b <- d |> 
  filter(wood_class == "S") |> #summary()
  #ggplot(aes(year, n, group = site)) + geom_col() + facet_wrap(~site)  
  #filter(site == "STA003") |> 
  filter(site == "STA028") |> 
  pull(n) |> 
  Rbeast::beast(season = "none")
b
plot(b)

#90 fits in a few seconds
b <- d |> 
  nest(.by = c(cmplx_strm, cmplx_abrv, cmplx_desc, strm, site, wood_class)) |> 
  #filter(str_detect(site, "ABR")) |> 
  filter(str_detect(site, "STA")) |> 
  mutate(
    bf = map(
      data, 
      ~Rbeast::beast(.x$n, season = "none", quiet = 1)
      )
  )
...
bb <- b |> 
  mutate(bf = map(bf, unclass)) |> 
  hoist(.col = bf, "marg_lik", "R2", "RMSE",
        ncp_median = list("trend","ncp_median")
        ,
        ncpPr0 = list("trend","ncpPr", 1),
        ncpPr1 = list("trend","ncpPr", 2),
        ncpPr2 = list("trend","ncpPr", 3)
        )

#GOF (captions for ABR, looks different for STA)
{
  bb |> 
  ggplot(aes(wood_class, marg_lik, group = site)) +
  geom_point(aes(color = wood_class), show.legend = F) + 
  geom_line(linewidth = 0.1, show.legend = F) +
  labs(
#    subtitle = "Fit marginal likelihood mostly relatively consistent across wood size classes per-site?"
  )
}/{
bb |> 
  ggplot(aes(wood_class, R2, group = site)) +
  geom_point(aes(color = wood_class), show.legend = F) + 
  geom_line(linewidth = 0.1, show.legend = F) +
  labs(
#    subtitle = "But not R2, though unclear what that means here..."
  )
}/{
bb |> 
  ggplot(aes(marg_lik, R2, group = site)) +
  geom_point(aes(color = wood_class), show.legend = F) + 
  facet_wrap(~wood_class) +
    labs(
#      subtitle = "But generally pos corr in GOF stats, esp for M & L"
    )
}

#this is nice
bb |> 
  select(site, wood_class, marg_lik, contains("ncpPr")) |> 
  pivot_longer(contains("ncpPr"), names_to = "ncpPr", values_to = "Pr") |> 
  ggplot() +
  geom_col(aes(Pr, site, fill = ncpPr, alpha = marg_lik)) +
  facet_wrap(~wood_class) +
  labs(
    subtitle = "Per-site, per-class prob. {0|1|2} change points\ndarker = better GOF"
  )
